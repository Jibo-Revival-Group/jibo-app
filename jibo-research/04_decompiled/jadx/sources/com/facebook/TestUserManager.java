...
public class TestUserManager {
    ...
    private AccessToken getAccessTokenForUser(List<String> list, Mode mode, String str) {
        JSONObject jSONObjectFindOrCreateSharedTestAccount;
        retrieveTestAccountsForAppIfNeeded();
        List<String> listAsList = Utility.isNullOrEmpty(list) ? Arrays.asList("email", "publish_actions") : list;
        if (mode == Mode.PRIVATE) {
            jSONObjectFindOrCreateSharedTestAccount = createTestAccount(listAsList, mode, str);
        } else {
            jSONObjectFindOrCreateSharedTestAccount = findOrCreateSharedTestAccount(listAsList, mode, str);
        }
        return new AccessToken(jSONObjectFindOrCreateSharedTestAccount.optString("access_token"), this.testApplicationId, jSONObjectFindOrCreateSharedTestAccount.optString(ShareConstants.WEB_DIALOG_PARAM_ID), listAsList, null, AccessTokenSource.TEST_USER, null, null);
    }
    ...
    private synchronized void retrieveTestAccountsForAppIfNeeded() {
        if (this.appTestAccounts == null) {
            ...
            List<GraphResponse> listExecuteBatchAndWait = GraphRequest.executeBatchAndWait(graphRequest, graphRequest2);
            if (listExecuteBatchAndWait == null || listExecuteBatchAndWait.size() != 2) {
                throw new FacebookException("Unexpected number of results from TestUsers batch query");
            }
            populateTestAccounts(listExecuteBatchAndWait.get(0).getJSONObject().optJSONArray(ShareConstants.WEB_DIALOG_PARAM_DATA), listExecuteBatchAndWait.get(1).getJSONObject());
        }
    }
    ...
    private synchronized void populateTestAccounts(JSONArray jSONArray, JSONObject jSONObject) {
        for (int i = 0; i < jSONArray.length(); i++) {
            ...
            try {
                jSONObjectOptJSONObject.put("name", jSONObject.optJSONObject(jSONObjectOptJSONObject.optString(ShareConstants.WEB_DIALOG_PARAM_ID)).optString("name"));
            } catch (JSONException e) {
                Log.e(LOG_TAG, "Could not set name", e);
            }
            storeTestAccount(jSONObjectOptJSONObject);
        }
    }
    ...
    private synchronized JSONObject findTestAccountMatchingIdentifier(String str) {
        ...
        while (true) {
            ...
            next = it.next();
            if (next.optString("name").contains(str)) {
                break;
            }
        }
        return next;
    ...
    private JSONObject findOrCreateSharedTestAccount(List<String> list, Mode mode, String str) {
        JSONObject jSONObjectFindTestAccountMatchingIdentifier = findTestAccountMatchingIdentifier(getSharedTestAccountIdentifier(list, str));
        return jSONObjectFindTestAccountMatchingIdentifier != null ? jSONObjectFindTestAccountMatchingIdentifier : createTestAccount(list, mode, str);
    }

    private String getSharedTestAccountIdentifier(List<String> list, String str) {
        return validNameStringFromInteger((str != null ? ((long) str.hashCode()) & 4294967295L : 0L) ^ (((long) getPermissionsString(list).hashCode()) & 4294967295L));
    }
    ...
    private JSONObject createTestAccount(List<String> list, Mode mode, String str) {
        ...
        GraphResponse graphResponseExecuteAndWait = new GraphRequest(null, String.format("%s/accounts/test-users", this.testApplicationId), bundle, HttpMethod.POST).executeAndWait();
