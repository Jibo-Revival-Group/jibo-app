package com.salesforce.android.cases.core.internal.operations;

import com.salesforce.android.cases.core.internal.remote.RemoteRepository;
import com.salesforce.android.cases.core.model.CreateCaseRecordResult;
import com.salesforce.android.cases.core.requests.CreateCaseRecordRequest;
import com.salesforce.android.service.common.utilities.control.Async;

/* JADX INFO: loaded from: classes.dex */
class CreateCaseRecordOp extends CaseFetchSaveOperation<CreateCaseRecordRequest, CreateCaseRecordResult> {
    public CreateCaseRecordOp(CreateCaseRecordRequest createCaseRecordRequest, RemoteRepository remoteRepository) {
        super(createCaseRecordRequest, null, remoteRepository);
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // com.salesforce.android.cases.core.internal.operations.CaseFetchSaveOperation
    /* JADX INFO: renamed from: a, reason: merged with bridge method [inline-methods] */
    public Async<CreateCaseRecordResult> mo12510a(RemoteRepository remoteRepository, CreateCaseRecordRequest createCaseRecordRequest) {
        return remoteRepository.m12599a(createCaseRecordRequest);
    }

    @Override // com.salesforce.android.cases.core.internal.operations.CaseFetchSaveOperation
    /* JADX INFO: renamed from: b */
    protected String mo12514b() {
        return "Cannot Create Case, Offline";
    }
}
