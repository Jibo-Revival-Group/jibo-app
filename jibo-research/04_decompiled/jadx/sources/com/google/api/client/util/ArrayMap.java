...
public class ArrayMap<K, V> extends AbstractMap<K, V> implements Cloneable {
    ...
    public static <K, V> ArrayMap<K, V> m8975of(Object... objArr) {
        ArrayMap<K, V> arrayMapCreate = create(1);
        int length = objArr.length;
        if (1 == length % 2) {
            String strValueOf = String.valueOf(String.valueOf(objArr[length - 1]));
            throw new IllegalArgumentException(new StringBuilder(strValueOf.length() + 28).append("missing value for last key: ").append(strValueOf).toString());
        }
