...
public class CropImageView extends TransformImageView {
    ...
    public void setImageToWrapCropBounds(boolean z) {
        if (this.mBitmapLaidOut && !isImageWrapCropBounds()) {
            ...
            float fCenterX = this.mCropRect.centerX() - f;
            float fCenterY = this.mCropRect.centerY() - f2;
            float fMax = DEFAULT_ASPECT_RATIO;
            ...
            if (zIsImageWrapCropBounds) {
                ...
                fCenterX = -(fArrCalculateImageIndents[0] + fArrCalculateImageIndents[2]);
                fCenterY = -(fArrCalculateImageIndents[3] + fArrCalculateImageIndents[1]);
            } else {
            ...
            if (z) {
                WrapCropBoundsRunnable wrapCropBoundsRunnable = new WrapCropBoundsRunnable(this, this.mImageToWrapCropBoundsAnimDuration, f, f2, fCenterX, fCenterY, currentScale, fMax, zIsImageWrapCropBounds);
                this.mWrapCropBoundsRunnable = wrapCropBoundsRunnable;
                post(wrapCropBoundsRunnable);
            } else {
                postTranslate(fCenterX, fCenterY);
                if (!zIsImageWrapCropBounds) {
    ...
    private float[] calculateImageIndents() {
        ...
        fArr[0] = f2;
        fArr[1] = f3 > DEFAULT_ASPECT_RATIO ? f3 : 0.0f;
        fArr[2] = f4 < DEFAULT_ASPECT_RATIO ? f4 : 0.0f;
    ...
    protected void onImageLaidOut() {
        ...
        if (drawable != null) {
            ...
            if (i > this.mThisHeight) {
                this.mCropRect.set((this.mThisWidth - ((int) (this.mThisHeight * this.mTargetAspectRatio))) / 2, DEFAULT_ASPECT_RATIO, r2 + r3, this.mThisHeight);
            } else {
                this.mCropRect.set(DEFAULT_ASPECT_RATIO, (this.mThisHeight - i) / 2, this.mThisWidth, i + r3);
            }
    ...
    private void setupInitialImagePosition(float f, float f2) {
        ...
        float f3 = ((fWidth - (this.mMinScale * f)) / 2.0f) + this.mCropRect.left;
        float f4 = ((fHeight - (this.mMinScale * f2)) / 2.0f) + this.mCropRect.top;
    ...
    protected void processStyledAttributes(TypedArray typedArray) {
        float fAbs = Math.abs(typedArray.getFloat(C1735R.styleable.ucrop_UCropView_ucrop_aspect_ratio_x, DEFAULT_ASPECT_RATIO));
        float fAbs2 = Math.abs(typedArray.getFloat(C1735R.styleable.ucrop_UCropView_ucrop_aspect_ratio_y, DEFAULT_ASPECT_RATIO));
