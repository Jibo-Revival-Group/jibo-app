...
public class Engine implements EngineJobListener, EngineResource.ResourceListener, MemoryCache.ResourceRemovedListener {
    ...
    public static class LoadStatus {
        ...
        private final ResourceCallback f4883cb;
        private final EngineJob engineJob;

        public LoadStatus(ResourceCallback resourceCallback, EngineJob engineJob) {
            this.f4883cb = resourceCallback;
            this.engineJob = engineJob;
        }

        public void cancel() {
            this.engineJob.removeCallback(this.f4883cb);
        }
    }
    ...
    public <T, Z, R> LoadStatus load(Key key, int i, int i2, DataFetcher<T> dataFetcher, DataLoadProvider<T, Z> dataLoadProvider, Transformation<Z> transformation, ResourceTranscoder<Z, R> resourceTranscoder, Priority priority, boolean z, DiskCacheStrategy diskCacheStrategy, ResourceCallback resourceCallback) {
        ...
        EngineKey engineKeyM5425a = this.keyFactory.m5425a(dataFetcher.getId(), key, i, i2, dataLoadProvider.getCacheDecoder(), dataLoadProvider.getSourceDecoder(), transformation, dataLoadProvider.getEncoder(), resourceTranscoder, dataLoadProvider.getSourceEncoder());
        EngineResource<?> engineResourceLoadFromCache = loadFromCache(engineKeyM5425a, z);
        if (engineResourceLoadFromCache != null) {
            resourceCallback.onResourceReady(engineResourceLoadFromCache);
            if (Log.isLoggable("Engine", 2)) {
                logWithTimeAndKey("Loaded resource from cache", jM5581a, engineKeyM5425a);
            }
            return null;
        }
        EngineResource<?> engineResourceLoadFromActiveResources = loadFromActiveResources(engineKeyM5425a, z);
        if (engineResourceLoadFromActiveResources != null) {
            resourceCallback.onResourceReady(engineResourceLoadFromActiveResources);
            if (Log.isLoggable("Engine", 2)) {
                logWithTimeAndKey("Loaded resource from active resources", jM5581a, engineKeyM5425a);
            }
            return null;
        }
        EngineJob engineJob = this.jobs.get(engineKeyM5425a);
        if (engineJob != null) {
            engineJob.addCallback(resourceCallback);
            if (Log.isLoggable("Engine", 2)) {
                logWithTimeAndKey("Added to existing load", jM5581a, engineKeyM5425a);
            }
            return new LoadStatus(resourceCallback, engineJob);
        }
        EngineJob engineJobBuild = this.engineJobFactory.build(engineKeyM5425a, z);
        EngineRunnable engineRunnable = new EngineRunnable(engineJobBuild, new DecodeJob(engineKeyM5425a, i, i2, dataFetcher, dataLoadProvider, transformation, resourceTranscoder, this.diskCacheProvider, diskCacheStrategy, priority), priority);
        this.jobs.put(engineKeyM5425a, engineJobBuild);
        engineJobBuild.addCallback(resourceCallback);
        engineJobBuild.start(engineRunnable);
        if (Log.isLoggable("Engine", 2)) {
            logWithTimeAndKey("Started new load", jM5581a, engineKeyM5425a);
        }
        return new LoadStatus(resourceCallback, engineJobBuild);
    }

    private static void logWithTimeAndKey(String str, long j, Key key) {
        Log.v("Engine", str + " in " + LogTime.m5580a(j) + "ms, key: " + key);
    }
