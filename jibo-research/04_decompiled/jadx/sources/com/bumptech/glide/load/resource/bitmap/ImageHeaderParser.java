...
public class ImageHeaderParser {
    ...
    private byte[] getExifSegment() throws IOException {
        ...
        do {
            ...
            if (uInt8 != 225) {
                jSkip = this.streamReader.skip(uInt16);
            } else {
                ...
                Log.d("ImageHeaderParser", "Unable to read segment data, type: " + ((int) uInt8) + ", length: " + uInt16 + ", actually read: " + i);
                return null;
            }
        } while (jSkip == uInt16);
        if (!Log.isLoggable("ImageHeaderParser", 3)) {
    ...
    private static class StreamReader {
        ...
        private final InputStream f4888is;

        public StreamReader(InputStream inputStream) {
            this.f4888is = inputStream;
        }

        public int getUInt16() throws IOException {
            return ((this.f4888is.read() << 8) & 65280) | (this.f4888is.read() & 255);
        }

        public short getUInt8() throws IOException {
            return (short) (this.f4888is.read() & 255);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long jSkip = this.f4888is.skip(j2);
                if (jSkip > 0) {
                    j2 -= jSkip;
                } else {
                    if (this.f4888is.read() == -1) {
                        break;
                    }
                    j2--;
                }
        ...
        public int read(byte[] bArr) throws IOException {
            ...
            while (length > 0) {
                int i = this.f4888is.read(bArr, bArr.length - length, length);
                if (i == -1) {
                    break;
                }
                length -= i;
