...
public class RectUtils {
   ...
   public static float[] getRectSidesFromCorners(float[] var0) {
      return new float[]{
         (float)Math.sqrt(Math.pow(var0[0] - var0[2], 2.0) + Math.pow(var0[1] - var0[3], 2.0)),
         (float)Math.sqrt(Math.pow(var0[2] - var0[4], 2.0) + Math.pow(var0[3] - var0[5], 2.0))
      };
   }
   ...
   public static RectF trapToRect(float[] var0) {
      ...
      for (byte var4 = 1; var4 < var0.length; var4 += 2) {
         float var2 = var0[var4 - 1];
         float var1 = var0[var4];
         float var3;
         if (var2 < var5.left) {
            var3 = var2;
         } else {
