.class abstract Lcom/google/common/util/concurrent/SmoothRateLimiter;
.super Lcom/google/common/util/concurrent/RateLimiter;
.source "SmoothRateLimiter.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Lcom/google/common/util/concurrent/SmoothRateLimiter$SmoothWarmingUp;
    }
.end annotation


# instance fields
.field a:D

.field b:D

.field c:D

.field private d:J


# direct methods
.method private constructor <init>(Lcom/google/common/util/concurrent/RateLimiter$SleepingStopwatch;)V
    .locals 2

    .prologue
    .line 334
    invoke-direct {p0, p1}, Lcom/google/common/util/concurrent/RateLimiter;-><init>(Lcom/google/common/util/concurrent/RateLimiter$SleepingStopwatch;)V

    .line 331
    const-wide/16 v0, 0x0

    iput-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    .line 335
    return-void
.end method

.method synthetic constructor <init>(Lcom/google/common/util/concurrent/RateLimiter$SleepingStopwatch;Lcom/google/common/util/concurrent/SmoothRateLimiter$1;)V
    .locals 0

    .prologue
    .line 25
    invoke-direct {p0, p1}, Lcom/google/common/util/concurrent/SmoothRateLimiter;-><init>(Lcom/google/common/util/concurrent/RateLimiter$SleepingStopwatch;)V

    return-void
.end method


# virtual methods
.method abstract a(DD)V
.end method

.method final a(DJ)V
    .locals 5

    .prologue
    .line 339
    invoke-virtual {p0, p3, p4}, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a(J)V

    .line 340
    sget-object v0, Ljava/util/concurrent/TimeUnit;->SECONDS:Ljava/util/concurrent/TimeUnit;

    const-wide/16 v2, 0x1

    invoke-virtual {v0, v2, v3}, Ljava/util/concurrent/TimeUnit;->toMicros(J)J

    move-result-wide v0

    long-to-double v0, v0

    div-double/2addr v0, p1

    .line 341
    iput-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->c:D

    .line 342
    invoke-virtual {p0, p1, p2, v0, v1}, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a(DD)V

    .line 343
    return-void
.end method

.method a(J)V
    .locals 7

    .prologue
    .line 391
    iget-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    cmp-long v0, p1, v0

    if-lez v0, :cond_0

    .line 392
    iget-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    sub-long v0, p1, v0

    long-to-double v0, v0

    invoke-virtual {p0}, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d()D

    move-result-wide v2

    div-double/2addr v0, v2

    .line 393
    iget-wide v2, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->b:D

    iget-wide v4, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    add-double/2addr v0, v4

    invoke-static {v2, v3, v0, v1}, Ljava/lang/Math;->min(DD)D

    move-result-wide v0

    iput-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    .line 394
    iput-wide p1, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    .line 396
    :cond_0
    return-void
.end method

.method final b()D
    .locals 4

    .prologue
    .line 349
    sget-object v0, Ljava/util/concurrent/TimeUnit;->SECONDS:Ljava/util/concurrent/TimeUnit;

    const-wide/16 v2, 0x1

    invoke-virtual {v0, v2, v3}, Ljava/util/concurrent/TimeUnit;->toMicros(J)J

    move-result-wide v0

    long-to-double v0, v0

    iget-wide v2, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->c:D

    div-double/2addr v0, v2

    return-wide v0
.end method

.method abstract b(DD)J
.end method

.method final b(IJ)J
    .locals 10

    .prologue
    .line 359
    invoke-virtual {p0, p2, p3}, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a(J)V

    .line 360
    iget-wide v0, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    .line 361
    int-to-double v2, p1

    iget-wide v4, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    invoke-static {v2, v3, v4, v5}, Ljava/lang/Math;->min(DD)D

    move-result-wide v2

    .line 362
    int-to-double v4, p1

    sub-double/2addr v4, v2

    .line 363
    iget-wide v6, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    .line 364
    invoke-virtual {p0, v6, v7, v2, v3}, Lcom/google/common/util/concurrent/SmoothRateLimiter;->b(DD)J

    move-result-wide v6

    iget-wide v8, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->c:D

    mul-double/2addr v4, v8

    double-to-long v4, v4

    add-long/2addr v4, v6

    .line 367
    iget-wide v6, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    invoke-static {v6, v7, v4, v5}, Lcom/google/common/math/LongMath;->a(JJ)J

    move-result-wide v4

    iput-wide v4, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->d:J

    .line 368
    iget-wide v4, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    sub-double v2, v4, v2

    iput-wide v2, p0, Lcom/google/common/util/concurrent/SmoothRateLimiter;->a:D

    .line 369
    return-wide v0
.end method

.method abstract d()D
.end method
